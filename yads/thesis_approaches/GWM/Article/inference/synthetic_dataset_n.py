...
def main():
    for seed, sample_num in enumerate(sample_dirs):
        ...
        for i, rota in enumerate(rota_files):
            ...
            S_local = S[cells_d]
            # shape prep
            q_flat_zeros = np.zeros((Nx_loc * Ny_loc))
            q_flat_zeros[well_loc_idx] = -np.log10(-q_pow[i])
            log_q = torch.from_numpy(np.reshape(q_flat_zeros, (Nx_loc, Ny_loc, 1)))
            log_dt = torch.from_numpy(np.full((Nx_loc, Ny_loc, 1), np.log(dt_init[i])))
            S_n = torch.from_numpy(np.array(np.reshape(S_local, (Nx_loc, Ny_loc, 1))))
            P_imp_local_n = torch.from_numpy(
                np.array(np.reshape(P_imp_local, (Nx_loc, Ny_loc, 1)))
            )
            ...
            x = x.reshape(1, Nx_loc, Ny_loc, 4)
            S_pred = model(x)
            S_pred = S_pred.detach().numpy()

            S_pred = np.reshape(S_pred, (Nx_loc * Ny_loc))
...
if __name__ == "__main__":
    ...
    folder_path = "nb_samples_3000_nb_boundaries_3_size_27_27/"
