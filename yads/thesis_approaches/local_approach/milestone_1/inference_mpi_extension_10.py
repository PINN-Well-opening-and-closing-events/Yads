...
def launch_inference(qt, log_qt, i):
    ...
    well_co2 = Well(
        name="well co2",
        cell_group=np.array([[20 * 51 / 4, 20 * 51 / 4]]),
        radius=0.1,
    ...
    cells_d = grid.find_cells_inside_square(
        (20 * (12 - ext), 20 * (12 + 1 + ext)), (20 * (12 + 1 + ext), 20 * (12 - ext))
    )
    print(len(cells_d))
    ...
    ax6.imshow(S_hybrid.reshape(51, 51).T)
...
if __name__ == "__main__":
    ...
    ext = 10
    if rank == 0:
        test_full = pd.read_csv(f"test_well_extension_10.csv", sep="\t", nrows=1)
    ...
    S_model.load_state_dict(
        torch.load(
            f"models/well_extension_{ext}/checkpoint_best_model_10_local_2d_1000.pt",
