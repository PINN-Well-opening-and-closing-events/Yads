...
from yads.thesis_approaches.local_approach.milestone_05.utils import (
    dict_to_args,
    data_dict_to_combi,
...
def main():
    ...
    save_dir = "sci_pres/data"
    nb_data = 2000
    np.random.seed(42)
    lhd = lhs(4, samples=nb_data, criterion="maximin")
    ...
    Pb_left = 100e5 + lhd[:, 2] * 80e5
    Pb_right = 100e5 + lhd[:, 3] * 80e5

    # rounding to avoid saving issues
    q_pow = np.round(q_pow, 6)
    dt_init = np.round(dt_init, 1)
    Pb_left = np.round(Pb_left, 0)
    Pb_right = np.round(Pb_right, 0)
    var_dict = {
        "dt_init": dt_init,
        "q": q_pow,
        "Pb_left": Pb_left,
    ...
    for i, key in enumerate(var_dict.keys()):
        ...
        # q -> well flux
        elif key in ["q", "S", "Pb_left", "Pb_right"]:
            if key == "q":
                well_ref = None
                other_wells = []
