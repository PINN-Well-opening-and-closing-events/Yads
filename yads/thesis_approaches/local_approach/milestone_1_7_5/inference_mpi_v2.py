...
def launch_inference(qt, log_qt, S_bounds, i):
    dict_save = {
        "q": qt[0],
        "total_sim_time": qt[1],
        "S0": qt[2],
    ...
    S_hybrid = copy.deepcopy(np.array(qt[2]))
    S_hybrid[cells_d] = S_pred
    dict_save["S_predict_global"] = S_hybrid.tolist()

    ###############################
    # Debug plot
    fig, (ax1, ax2, ax3, ax4, ax5, ax6) = plt.subplots(1, 6, figsize=(20, 12))
    ...
    ax2.imshow(np.array(qt[2]).reshape(95, 60).T, vmin=0, vmax=1)
    ax3.imshow(S_plot.reshape(local_shape, local_shape).T, vmin=0, vmax=1)
    ax4.imshow(np.power(P_imp_plot.reshape(local_shape, local_shape).T, 10))
    ax5.imshow(S_pred.reshape(local_shape, local_shape).T, vmin=0, vmax=1)
    ax6.imshow(S_hybrid.reshape(95, 60).T, vmin=0, vmax=1)
    fig.axes[1].invert_yaxis()
    fig.axes[2].invert_yaxis()
    fig.axes[0].invert_yaxis()
    fig.axes[3].invert_yaxis()
    fig.axes[4].invert_yaxis()
    fig.axes[5].invert_yaxis()
    plt.show()
    ################################
...
def main():
    ...
    qts = test[["q", "dt_init", "S0", "P_imp"]].to_numpy()
    log_qts = test[["log_q", "log_dt", "S0", "P_imp"]].to_numpy()
    S_bounds = test["S_boundary"].values

    for i in range(len(test)):
        result = launch_inference(
            qt=qts[i], log_qt=log_qts[i], S_bounds=S_bounds[i], i=i
        )
...
if __name__ == "__main__":
    ...
    S_model = model = FNO2d(modes1=16, modes2=16, width=128, n_features=4)
    S_model.load_state_dict(
        torch.load(
            "models/well_extension_10/checkpoint_best_model_10_local_2d_500.pt",
            map_location=torch.device("cpu"),
        )
    )
    q_normalizer = pickle.load(open("models/well_extension_10/q_normalizer.pkl", "rb"))
    S_normalizer = pickle.load(open("models/well_extension_10/S_normalizer.pkl", "rb"))
    dt_normalizer = pickle.load(
        open("models/well_extension_10/dt_normalizer.pkl", "rb")
    )
    P_imp_normalizer = pickle.load(
        open("models/well_extension_10/P_imp_normalizer.pkl", "rb")
    )
    if rank == 0:
