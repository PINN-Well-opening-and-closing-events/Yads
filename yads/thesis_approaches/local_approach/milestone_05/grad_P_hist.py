...
def launch_inference(qt, Pb_dict):
...
def main():
    ...
    for i in range(len(test)):
        (
            ...
            left_grad_left,
            right_grad_left,
        ) = launch_inference(qt=qts[i], Pb_dict=Pb_dicts[i])
    ...
    df.to_csv(f"test_pressure_Grads.csv", sep="\t", index=False)
...
if __name__ == "__main__":
    ...
    if rank == 0:
        test_full = pd.read_csv(
            ...
            converters={"S_local": literal_eval, "Pb": literal_eval},
            sep="\t",
        )
