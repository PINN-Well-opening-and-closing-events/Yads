...
def solss_model_test_n_iter(
    ...
    assert total_time == total_sim_time
...
def main():
    ...
    df.to_csv("./data/quantification/result_quantification_110_100_P_test.csv")
    return
...
if __name__ == "__main__":
    ...
    classifier_scaler = pickle.load(
        open("models/case_classifier_scaler_xgboost_110_100_P.pkl", "rb")
    )
    classifier = pickle.load(
        open("models/case_classifier_model_xgboost_110_100_P.pkl", "rb")
    )

    dt_predict = pickle.load(
        open("models/dt_predict_model_xgboost_110_100_P.pkl", "rb")
    )
    dt_predict_scaler = pickle.load(
        open("models/dt_predict_scaler_xgboost_110_100_P.pkl", "rb")
    )

    S_predict = pickle.load(
        open("models/S_predict_model_hard_xgboost_110_100_P.pkl", "rb")
    )
    S_predict_scaler = pickle.load(
        open("models/S_predict_scaler_xgboost_110_100_P.pkl", "rb")
    )
