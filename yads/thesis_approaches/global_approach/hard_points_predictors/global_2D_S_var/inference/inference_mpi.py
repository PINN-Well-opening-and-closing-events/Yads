...
def launch_inference(qt, log_qt, S_bounds, i):
    ...
    Sb_dict["Dirichlet"] = {
        "injector_one": S_bounds,
        "injector_two": S_bounds,
        "right": 0.0,
    }
...
def main():
    ...
    S_bounds = test["S_boundary"].to_numpy()
    for i in range(len(test)):
        result = launch_inference(
            qt=qts[i], log_qt=log_qts[i], S_bounds=S_bounds[i][0], i=i
        )
...
if __name__ == "__main__":
    ...
    if rank == 0:
        test_full = pd.read_csv(
            "data/test_q_5_3_dt_1_10_v2.csv",
    ...
    S_model.load_state_dict(
        torch.load(
            "model/S_var_v1/best_model_global_2d_S_var_FNO_49000.pt",
            map_location=torch.device("cpu"),
        )
    )
    q_normalizer = pickle.load(open("model/S_var_v1/q_normalizer.pkl", "rb"))
    S_normalizer = pickle.load(open("model/S_var_v1/S_normalizer.pkl", "rb"))
    dt_normalizer = pickle.load(open("model/S_var_v1/dt_normalizer.pkl", "rb"))
    if rank == 0:
