...
def launch_inference(qt, log_qt, S_bounds, i):
    ...
    # Sb_dict['Dirichlet'] = {"injector_one": S_bounds, "injector_two": S_bounds, "right": 0.}
    Sb_dict = S_bounds
    # Data prep for model
    # shape prep
    ...
    P_imp = torch.from_numpy(np.array(np.reshape(log_qt[3], (95, 60, 1))))
    # normalizer prep
    log_q_n = q_normalizer.encode(log_q)
    log_dt_n = dt_normalizer.encode(log_dt)
    S_n = S_normalizer.encode(S_n)

    P_imp = P_normalizer.encode(P_imp)

    x = torch.cat([log_q_n, log_dt_n, S_n, P_imp], 2).float()
...
if __name__ == "__main__":
    ...
    if rank == 0:
        test_full = pd.read_csv(
            "data/light_train_q_5_3_dt_1_10_v2.csv",
            converters={
                "S0": literal_eval,
    ...
    S_model.load_state_dict(
        torch.load(
            "model/best_model_global_2d_S_var_v2_FNO_7300.pt",
    ...
    P_normalizer = pickle.load(open("model/P_imp_normalizer.pkl", "rb"))
    if rank == 0:
