...
from yads.predictors.hard_points_predictors.level_2.utils import (
    dict_to_args,
    dict_to_args_hard,
    data_dict_to_combi,
...
def find_hard_points(var_dict, data_dict, savepath: str):
    ...
    for i, (comb, state) in enumerate(simulation_dataset):
        for tot_t in state["data"].keys():
            ...
            if nb_newton > state["metadata"]["max_newton_iter"]:
                ...
                hard_var_dict["S"].append(var_dict["S"][i])
    ...
    ##### GIGA HARD SIM PART ###################
    # Newton > 100
    giga_hard_var_dict = {"dt_init": [], "q": [], "S": []}
    ...
    for i, (comb, state) in enumerate(hard_simulation_dataset):
        for tot_t in state["data"].keys():
            ...
            if nb_newton == 100:
                ...
                giga_hard_var_dict["S"].append(hard_var_dict["S"][i])
