...
def plot_S_P(S0, P0, P_imp, S_pred, i):
    ...
    ax1.imshow(P0.reshape(95, 60).T, cmap="viridis")
    fig.axes[0].invert_yaxis()
    im2 = ax2.imshow(P_imp.reshape(95, 60).T, cmap="viridis")
    fig.axes[1].invert_yaxis()
    fig.subplots_adjust(right=0.85)
    cbar_ax = fig.add_axes([0.88, 0.3, 0.04, 0.4])
    fig.colorbar(im2, cax=cbar_ax)
...
def launch_inference(qt, log_qt, i, test_P, test_S):
    ...
    print("SOLUTION: ", nb_newton, norms)
    P_i_plus_1, S_i_plus_1, dt_sim, nb_newton, norms = hybrid_newton_inference(
        grid=grid,
        P=P_imp,
        S=qt[2],
    ...
    print(
        f"step {i}: Norms classic {dict_save['norms_classic']}, hybrid {dict_save['norms_hybrid']}"
    )
...
def main():
    ...
    print(test["nb_newton"][100:104])

    result = Parallel(n_jobs=4)(
        delayed(launch_inference)(
            qt=qts[i], log_qt=log_qts[i], i=i, test_P=test["P"][i], test_S=test["S"][i]
        )
        for i in range(101, 105)
    )
    df = pd.DataFrame(result)
    df.to_csv("./results/result_quantification_test_0_1.csv", sep="\t", index=False)
    return
