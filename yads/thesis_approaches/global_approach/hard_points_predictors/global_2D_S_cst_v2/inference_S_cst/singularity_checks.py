...
from yads.predictors.hard_points_predictors.global_2D_S_cst_v2.inference_S_cst.debug_newton_solss_v2 import (
    solss_newton_step_v2,
)
...
def hybrid_newton_inference(
    ...
    P_i_plus_1, S_i_plus_1, dt_sim, nb_newton, norms, B_list = solss_newton_step_v2(
        grid=grid,
        P_i=P_i,
        S_i=S_i,
        Pb=Pb,
    ...
    print(norms["L_inf"])
    print(norms["L2"])
    return P_i_plus_1, S_i_plus_1, dt_sim, nb_newton, norms, B_list


def launch_inference(qt, log_qt, P_sols, i):
    ...
    P_i_plus_1, S_i_plus_1, dt_sim, nb_newton, norms, B_list = hybrid_newton_inference(
        grid=grid,
        P=P_imp,
        S=qt[2],
        Pb=Pb,
    ...
    dict_save["Residuals_classic"] = B_list
    ...
    dict_save["Residuals_hybrid"] = B_list
    ...
    dict_save["S_i_plus_1_P_sol_S_pred"] = S_i_plus_1.tolist()
    dict_save["P_i_plus_1_P_sol_S_pred"] = P_i_plus_1.tolist()
    dict_save["nb_newton_hybrid_P_sol_S_pred"] = nb_newton
    dict_save["dt_sim_P_sol_S_pred"] = dt_sim
    dict_save["norms_P_sol_S_pred"] = norms
    dict_save["Residuals_P_sol_S_pred"] = B_list
    ...
    dict_save["S_i_plus_1_P_sol_S"] = S_i_plus_1.tolist()
    dict_save["P_i_plus_1_P_sol_S0"] = P_i_plus_1.tolist()
    dict_save["nb_newton_hybrid_P_sol_S0"] = nb_newton
    dict_save["dt_sim_P_sol_S0"] = dt_sim
    dict_save["norms_P_sol_S0"] = norms
    dict_save["Residuals_P_sol_S0"] = B_list
...
def main():
    ...
    P_sols = test["P"].to_numpy()
    for i in range(len(test)):
        result = launch_inference(qt=qts[i], log_qt=log_qts[i], P_sols=P_sols[i], i=i)
        df = pd.DataFrame([result])
        df.to_csv(
            f"./results/quantification_test_{rank}_{len(test)}_{i}.csv",
            sep="\t",
            index=False,
        )
        if rank == 0:
