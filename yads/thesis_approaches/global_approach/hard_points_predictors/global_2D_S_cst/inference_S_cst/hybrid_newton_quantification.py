...
def launch_inference(qt, log_qt, i):
    ...
    print(
        f"classic res: {np.max(np.abs(X_classic_res) * qt[1] / np.concatenate([V, V]))}, "
        f"hybrid res: {np.max(np.abs(X_pred_res) * qt[1] / np.concatenate([V, V]))}"
    )
    ...
    plot_S_P(
        S_i_plus_1,
        X_classic_res[grid.nb_cells :],
        X_pred_res[grid.nb_cells :],
        S_pred,
        i,
    )
...
def main():
    ...
    print(test["nb_newton"][6])
    result = Parallel(n_jobs=4)(
        delayed(launch_inference)(qt=qts[i], log_qt=log_qts[i], i=i)
        for i in range(6, 7)
    )
    df = pd.DataFrame(result)
    df.to_csv("./results/result_quantification_test_6.csv", sep="\t", index=False)
    return
