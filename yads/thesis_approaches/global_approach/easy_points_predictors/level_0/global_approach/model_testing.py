...
def launch_inf(qt, knn_model_pred, cnn_model_pred, fno_model_pred, S_sol, i):
    ...
    dict_save = {
        ...
        "S_cnn": cnn_model_pred.tolist(),
        "S_fno": fno_model_pred.tolist(),
        "P_imp": None,
        "q": qt[0],
        "t": qt[1],
        "S0": qt[2],
    ...
    dict_save["dt_cnn"], dict_save["cnn_newtons"] = solss_model_test_1_iter(
        grid=grid,
        P=P,
        S=qt[2],
        Pb=Pb,
    ...
    dict_save["dt_fno"], dict_save["fno_newtons"] = solss_model_test_1_iter(
        grid=grid,
        P=P,
        S=qt[2],
        Pb=Pb,
...
def main():
    result = Parallel(n_jobs=4)(
        delayed(launch_inf)(
            qt=qt[i],
            cnn_model_pred=cnn_model_pred[i],
            knn_model_pred=knn_model_pred[i],
            fno_model_pred=fno_model_pred[i],
            S_sol=testY[i],
            i=i,
        )
    ...
    df.to_csv("global_approach/data/result_test_4000.csv")
    return


if __name__ == "__main__":
    knn_model_path = "global_approach/models/model_knn_dist.pkl"
    test = pd.read_csv(
        "global_approach/data/test_4000.csv",
        converters={"S": literal_eval, "P": literal_eval, "S0": literal_eval},
    )
    testX, testY = np.array(list(test["S0"])), np.array(list(test["S"]))

    qt = test[["q", "total_time", "S0"]].to_numpy()
    ...
    cnn_model = keras.models.load_model("global_approach/models/UNET")

    knn_model_pred = knn_model.predict(testX)
    cnn_model_pred = cnn_model.predict(testX).reshape((test.shape[0], grid.nb_cells))
    fno_model_pred = pd.read_csv(
        "global_approach/data/level_0_FNO_pred_test.csv",
        converters={"S": literal_eval, "FNO_pred": literal_eval, "S0": literal_eval},
    )
    fno_model_pred = np.array(list(fno_model_pred["FNO_pred"]))
    nb_iter = test.shape[0]
    del test
    del knn_model
    del cnn_model
    main()
