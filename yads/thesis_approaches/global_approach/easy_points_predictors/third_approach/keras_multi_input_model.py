...
df = pd.read_csv(
    "data/third_approach_dt_S_1000.csv",
...
scaler.fit(trainX[["q", "dt"]])
trainX[["q", "dt"]], testX[["q", "dt"]] = (
    scaler.transform(trainX[["q", "dt"]]),
    scaler.transform(testX[["q", "dt"]]),
)

trainAttrX, testAttrX = trainX[["q", "dt"]].to_numpy(), testX[["q", "dt"]].to_numpy()
...
model_cnn = create_cnn_1D(1, map_shape[1], 16, filters=(128, 64, 32, 16))
model_mlp = create_256_mlp(2, 16)

concatenated = concatenate([model_mlp.output, model_cnn.output])
model = create_multi_input_model(model_cnn, model_mlp, filters=(64, 128, 256))
