from yads.predictors.easy_points_predictors.second_approach.utils import (
    dict_to_args,
    data_dict_to_combi,
...
def make_pipeline_lhs(var_dict, data_dict, savepath: str):
    ...
    for i, key in enumerate(var_dict.keys()):
        if key in data_dict.keys():
            ...
            if key == "S":
                var_list_serializable.append(var_dict[key].tolist())
            else:
    ...
    # [q, dt, S]
    combinations = [
        (var_list[0][i], var_list[1][i], var_list[2][i])
    ...
    simulation_dataset = Parallel(n_jobs=4)(
        delayed(launch_sim)(
