...
def test_wrong_inputs():
    ...
    mu_wrong, mu_gk = -1.0, 1.0
    with pytest.raises(ValueError, match=r"viscosity must be positive"):
        total_mobility(1.0, mu_w=mu_wrong, mu_g=mu_gk)
        total_mobility(1.0, mu_w=mu_wrong, mu_g=mu_gk, model="quadratic")
    ...
    with pytest.raises(ValueError, match=r"viscosity must be positive"):
        calculate_mobility(grid, P, S, Pb, Sb_dict, mu_w=mu_wrong, mu_g=mu_gk)

    P_wrong = S_wrong = np.ones(grid.nb_faces)

    with pytest.raises(ValueError, match=r"viscosity must be positive"):
        d_total_mobility_ds(sw=S, mu_w=mu_wrong, mu_g=mu_gk, model="cross")

    with pytest.raises(ValueError, match=r"Model not handled yet"):
        d_total_mobility_ds(sw=S, mu_w=mu_gk, mu_g=mu_gk, model="error")
...
def test_output():
    ...
    Sb_n = {"left": None, "right": None}
    Sb_dict = {"Dirichlet": Sb_d, "Neumann": Sb_n}
    calculate_mobility(grid, P, S, Pb, Sb_dict, mu_w=mu_gk, mu_g=mu_gk)
