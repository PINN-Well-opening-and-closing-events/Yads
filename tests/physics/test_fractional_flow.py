...
def test_wrong_inputs():
    ...
    with pytest.raises(ValueError, match=r"Saturation sw must be between 0 and 1"):
        fw(sw=sw_down, mu_g=1.0, mu_w=1.0)
    with pytest.raises(ValueError, match=r"Saturation sw must be between 0 and 1"):
        fw(sw=sw_up, mu_g=1.0, mu_w=1.0)

    with pytest.raises(ValueError, match=r"Saturation sw must be between 0 and 1"):
        dfw_dsw(sw=sw_down, mu_g=1.0, mu_w=1.0)
    with pytest.raises(ValueError, match=r"Saturation sw must be between 0 and 1"):
        dfw_dsw(sw=sw_up, mu_g=1.0, mu_w=1.0)
    ...
    with pytest.raises(ValueError, match=r"viscosity must be positive"):
        fw(1.0, mu_w=mu_wrong, mu_g=mu_gk)
